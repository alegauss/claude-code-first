...
| `python scripts/traceability.py --check` | every rule traces to findings its keyword is admitted by, every finding to a source; without `--check` it rewrites spec/traceability.md |
...
CI runs the same eleven on every push (`.github/workflows/`), so a local green that CI
