...
- 📋 **CCF1** (deps: —) **a session opening this repository is told nothing: no every-turn file states purpose, language or write paths** — Each session rediscovers what the repository is and that it is written in English, which is the cost the specification exists to remove. → §CCF1
